...
impl FollowCamera {
    ...
    pub fn set_free(&mut self, pos: Vector3, yaw: f32, pitch: f32) {
        self.mode = Mode::Free;
        self.pos = pos;
        // Convention: yaw=0 -> forward = +Z (matches the existing follow camera,
        // which places itself at pivot -Z*dist at yaw=0). So target = pos + (sin(yaw), 0, cos(yaw)) when pitch=0.
        let cp = pitch.cos();
        let sp = pitch.sin();
        self.target = Vector3 {
            x: pos.x + yaw.sin() * cp,
    ...
    pub fn update_free(&mut self, input: &crate::input::Input, dt: f32) {
        ...
        self.target = Vector3 {
            x: self.pos.x + sy * cp,  // forward.x = sin(yaw)
            y: self.pos.y + sp,
            z: self.pos.z + cy * cp,  // forward.z = cos(yaw)
        };
    }
}
```
...
impl<'a> Game<'a> {
    ...
    pub fn enter_test_mode(&mut self, args: crate::cli_args::Args) {
        ...
        // The city is already fully generated by `Game::new` (cfg.city_blocks=10
        // centered on origin). The blocks around the player's default (0,0,0) are
        // present from construction. No need to call `ensure_blocks_around` here.
        // The preset is responsible for adding any extra shops/pickups if needed.
